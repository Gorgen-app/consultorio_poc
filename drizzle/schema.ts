...
// ==========================================
// MÓDULO DE AGENDAMENTO CIRÚRGICO
// Fluxo completo: Solicitação → Autorização → Confirmação → Realização
// ==========================================

/**
 * Status da Solicitação Cirúrgica
 */
export const statusSolicitacaoCirurgicaEnum = mysqlEnum("status_solicitacao_cirurgica", [
  "Pendente de Guias",      // Aguardando geração das guias
  "Guias Geradas",          // Guias prontas para envio
  "Guias Enviadas",         // Guias enviadas ao hospital/convênio
  "Em Análise",             // Convênio analisando
  "Autorizada",             // Convênio autorizou
  "Negada",                 // Convênio negou
  "Confirmada",             // Data e horário confirmados com hospital
  "Realizada",              // Cirurgia realizada
  "Cancelada"               // Solicitação cancelada
]);

/**
 * Tabela de Solicitações Cirúrgicas
 * Gerencia todo o fluxo de agendamento cirúrgico
 */
export const solicitacoesCirurgicas = mysqlTable("solicitacoes_cirurgicas", {
  id: int("id").autoincrement().primaryKey(),
  tenantId: int("tenant_id").notNull().references(() => tenants.id),
  
  // Identificador único (formato: SC-YYYY-NNNNN)
  idSolicitacao: varchar("id_solicitacao", { length: 20 }).notNull().unique(),
  ...
  // Procedimento
  procedimento: varchar("procedimento", { length: 500 }).notNull(),
  codigoProcedimento: varchar("codigo_procedimento", { length: 50 }), // Código TUSS/CBHPM
  lateralidade: mysqlEnum("lateralidade", ["Direita", "Esquerda", "Bilateral", "N/A"]).default("N/A"),
  ...
  dataPrevista: date("data_prevista"),
  horaPrevistoInicio: varchar("hora_previsto_inicio", { length: 5 }), // HH:MM
  ...
  // Materiais e OPME
  materiaisEspeciais: text("materiais_especiais"), // JSON com lista de materiais
  opme: text("opme"), // JSON com OPME necessário
  ...
  guiasEnviadasEm: timestamp("guias_enviadas_em"),
  guiasEnviadasPara: varchar("guias_enviadas_para", { length: 255 }), // Email do hospital
  
  // Autorização
  numeroAutorizacao: varchar("numero_autorizacao", { length: 50 }),
  dataAutorizacao: timestamp("data_autorizacao"),
  validadeAutorizacao: date("validade_autorizacao"),
  observacoesAutorizacao: text("observacoes_autorizacao"),
  ...
  // Status
  status: statusSolicitacaoCirurgicaEnum.default("Pendente de Guias").notNull(),
  
  // Observações
  indicacaoCirurgica: text("indicacao_cirurgica"),
  observacoes: text("observacoes"),
  
  // Vínculo com agendamento (quando confirmada)
  agendamentoId: int("agendamento_id").references(() => agendamentos.id),
  
  // Vínculo com cirurgia (quando realizada)
  cirurgiaId: int("cirurgia_id").references(() => cirurgias.id),
...
export type SolicitacaoCirurgica = typeof solicitacoesCirurgicas.$inferSelect;
export type InsertSolicitacaoCirurgica = typeof solicitacoesCirurgicas.$inferInsert;

/**
 * Histórico de Status das Solicitações Cirúrgicas
...
export type HistoricoSolicitacaoCirurgica = typeof historicoSolicitacaoCirurgica.$inferSelect;
export type InsertHistoricoSolicitacaoCirurgica = typeof historicoSolicitacaoCirurgica.$inferInsert;

/**
 * Tabela de Profissionais/Médicos
 * Para filtrar agenda por profissional
 */
