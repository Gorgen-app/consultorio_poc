  ...
  googleCalendar: router({
    ...
    getConfig: tenantProcedure
      .query(async ({ ctx }) => {
        ...
        return await gcalDb.getGoogleCalendarConfig(ctx.tenant.tenantId, ctx.user.id);
      }),
    ...
    saveConfig: tenantProcedure
      .input(z.object({
        ...
        syncDirection: z.enum(["bidirectional", "to_google_only", "from_google_only"]).optional(),
        googleCalendarId: z.string().optional(),
        syncConsultas: z.boolean().optional(),
        syncCirurgias: z.boolean().optional(),
        syncReunions: z.boolean().optional(),
        syncBloqueios: z.boolean().optional(),
        syncOutros: z.boolean().optional(),
        includePatientName: z.boolean().optional(),
        includePatientPhone: z.boolean().optional(),
        eventVisibility: z.enum(["default", "public", "private"]).optional(),
      }))
      .mutation(async ({ input, ctx }) => {
        if (!ctx.user?.id) throw new Error("Usuário não autenticado");
        const gcalDb = await import('./google-calendar-db');
        return await gcalDb.upsertGoogleCalendarConfig(ctx.tenant.tenantId, ctx.user.id, input);
      }),

    // Listar registros de sincronização
    listSyncs: tenantProcedure
      .query(async ({ ctx }) => {
        const gcalDb = await import('./google-calendar-db');
        return await gcalDb.listAllSyncs(ctx.tenant.tenantId);
      }),
    ...
    listPending: tenantProcedure
      .query(async ({ ctx }) => {
        const gcalDb = await import('./google-calendar-db');
        return await gcalDb.listPendingSyncs(ctx.tenant.tenantId);
      }),

    // Obter status de sincronização de um agendamento
    getSyncStatus: tenantProcedure
      .input(z.object({ agendamentoId: z.number() }))
      .query(async ({ input, ctx }) => {
        const gcalDb = await import('./google-calendar-db');
        return await gcalDb.getSyncByAgendamento(ctx.tenant.tenantId, input.agendamentoId);
      }),

    // Criar registro de sincronização (após enviar para Google)
    createSyncRecord: tenantProcedure
      .input(z.object({
        agendamentoId: z.number(),
      ...
      .mutation(async ({ input, ctx }) => {
        const gcalDb = await import('./google-calendar-db');
        return await gcalDb.createSyncRecord(
          ctx.tenant.tenantId,
          input.agendamentoId,
          input.googleEventId,
    ...
    updateSyncStatus: tenantProcedure
      .input(z.object({
        syncId: z.number(),
        status: z.enum(["synced", "pending_to_google", "pending_from_google", "conflict", "error"]),
        direction: z.enum(["to_google", "from_google"]).optional(),
        errorMessage: z.string().optional(),
      }))
      .mutation(async ({ input }) => {
        const gcalDb = await import('./google-calendar-db');
        await gcalDb.updateSyncStatus(
          input.syncId,
          input.status,
          input.direction,
          input.errorMessage
